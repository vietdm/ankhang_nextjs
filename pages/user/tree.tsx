...
const UserTree = () => {
  ...
  const getTreeUser = () => {
    ...
    if (typeof historyUserTree[username] != "undefined") {
      setUserTree(null);
      setTimeout(() => setUserTree(historyUserTree[username]), 100);
      return;
    }
    setRequesting(true);
    fetch.get("/user/tree/" + username).then((result: any) => {
      ...
      setUserTree(null);
      setTimeout(() => setUserTree(result.data), 100);
      setRequesting(false);
          ...
          {queryUsername.length > 1 && !requesting && (
            ...
            >
              ...
              >
                ...
                <Typography component="p" marginLeft={1} sx={{ userSelect: "none" }}>Quay lại danh sách
                  trước</Typography>
              </Stack>
            </motion.div>
          )}
