...
const WithdrawPage = () => {
  ...
  const sendOtp = () => {
    if (!user) return;
    if (user.email == "") {
      Alert.error("Bạn cần cập nhật email trong phần Cập nhật thông tin cá nhân mới có thể gửi OTP");
      return;
    }
    setRequesting(true);
      ...
      <Stack marginTop={3}>
        {bankInfo == null && (
          <Box marginBottom={4}>
            <Typography
              ...
              fontStyle="italic"
            >
              Bạn cần vào cập nhật <b>Thông tin Ngân hàng</b> trong phần <b>Cập nhật thông tin cá nhân</b> mới có thể
              rút tiền
            </Typography>
            <Link passHref href="/user/edit?backhome=1">
              <Typography textAlign="center" paddingTop={1} color="#3498db">Bấm vào đây để đến cập nhật thông
                tin</Typography>
            </Link>
          </Box>
        )}
        ...
        <Box>
          ...
          <Typography component="p"
                      textAlign="right"><b>{!money ? 0 : formatMoney(money - money / 10)} đ</b></Typography>
        </Box>
        <HrTag p={2} />
        <Typography component="h4" textAlign="center" fontSize="20px" fontWeight={700} marginBottom={2}>Thông tin nhận
          tiền</Typography>
